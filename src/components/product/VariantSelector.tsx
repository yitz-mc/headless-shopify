...
const COLOR_STYLES: Record<string, React.CSSProperties> = {
  ...
  grey: {
    backgroundImage:
      'url(https://sfycdn.speedsize.com/eed8e0c4-8a58-46ce-832b-26984a0a57e6/cdn.shopify.com/s/files/1/0857/4644/files/Grey_1.png?v=1752767554)',
  ...
  'natural oak': {
    backgroundImage:
      'url(https://sfycdn.speedsize.com/eed8e0c4-8a58-46ce-832b-26984a0a57e6/cdn.shopify.com/s/files/1/0857/4644/files/Oak_1.png?v=1752767555)',
    backgroundSize: 'cover',
    backgroundRepeat: 'no-repeat',
  },
  'white / natural oak': {
    backgroundImage:
      'url(https://sfycdn.speedsize.com/eed8e0c4-8a58-46ce-832b-26984a0a57e6/cdn.shopify.com/s/files/1/0857/4644/files/White_natural_oak.png?v=1752768870)',
    backgroundSize: 'cover',
    backgroundRepeat: 'no-repeat',
  },
  black: {
  ...
  chrome: {
    background:
      'linear-gradient(90deg, #e0e0e0, #f9f9f9 20%, #c0c0c0 50%, #f9f9f9 80%, #e0e0e0), linear-gradient(0deg, rgba(255, 255, 255, 0.3), rgba(0, 0, 0, 0.1))',
  },
  'brushed nickel': {
    background:
      'linear-gradient(90deg, #d6d6d6, #e3e3e3 20%, #bfbfbf 50%, #e3e3e3 80%, #d6d6d6), linear-gradient(0deg, rgba(255, 255, 255, 0.2), rgba(0, 0, 0, 0.1))',
  },
  'satin nickel': {
    background:
      'linear-gradient(180deg, #cfc3b4 0%, #beb2a3 25%, #ddd3c5 50%, #b1a293 75%, #cfc3b4 100%)',
  },
  brass: {
    background:
      'linear-gradient(90deg, #f9e4b7, #fdf2d3 20%, #f2d28a 50%, #fdf2d3 80%, #f9e4b7), linear-gradient(0deg, rgba(255, 255, 255, 0.3), rgba(255, 215, 0, 0.1))',
  },
  'satin brass': {
    background:
      'linear-gradient(90deg, #f9e4b7, #fdf2d3 20%, #f2d28a 50%, #fdf2d3 80%, #f9e4b7), linear-gradient(0deg, rgba(255, 255, 255, 0.3), rgba(255, 215, 0, 0.1))',
  ...
  'matte black': {
    background:
      'linear-gradient(90deg, #3a3a3a 0%, #4a4a4a 20%, #5b5b5b 50%, #3e3e3e 80%, #2b2b2b 100%)',
  },
};
...
export function VariantSelector({
  ...
  const isOptionAvailable = (optionName: string, value: string): boolean => {
    ...
    return variants.some((variant) => {
      const matches = variant.selectedOptions.every((opt) => testOptions[opt.name] === opt.value);
  ...
  return (
    <div className='product-form__swatches'>
      {options.map((option) => {
        ...
        return (
          <div key={option.id} className='swatches__swatch mb-6'>
